...
class CycleMode(BaseFrame):
    def __init__(self, parent, controller):
        super().__init__(parent, controller)
        self.set_background('imgs/cycle_mode.png')
        # label = ttk.Label(self, text="Циклический режим", font=('Helvetica', 14))
        # label.pack(pady=20)
        ...
        self.current_speed = ttk.Label(self, style="Main.TLabel", text="0.0",
                                       textvariable=self.controller.speed_mpa_var)

        self.current_speed.place(x=655, y=177, width=68)

        self.start_automat_n3 = ttk.Button(self, text="Запуск автомат Н3", style="NonActive.TButton",
                                           command=lambda:
                                           self.start_mode_func(self.start_automat_n3, START_AUTOMAT_N3_CYCLE_REG))
        self.start_automat_n3.place(x=630, y=260, width=120, height=50)

        self.start_mode = ttk.Button(self, text="Запуск режима",
                                     command=lambda: self.start_mode_func(self.start_mode, START_MODE_CYCLE_REG),
        ...
        self.cycles = ttk.Label(self, style="Main.TLabel", text="0", textvariable=self.controller.number_of_cycles_var)
        self.cycles.place(x=655, y=97, width=68)
